...
namespace Swd3ModStudio.Prototype
{
    ...
    public static class Program
    {
        ...
        static readonly List<string> Commands = new List<string> { "health", "list", "details", "create-test", "update-test", "verify-test", "download-test", "self-test" };
        ...
        static List<Item> Query(UGCQueryHandle_t handle, out uint total)
        {
            ...
            try
            {
                ...
                string language = Options.ContainsKey("language") ? Options["language"] : "english";
        ...
        public static void ValidatePlan(TestPlan p)
        {
            ...
            Require(files.Length == 1 && Path.GetFileName(files[0]) == PackageName, "Content must contain only the M0 fixture package.");
        ...
        {
            string dir = Path.Combine(Path.GetTempPath(), "swd3-m0-tests-" + Guid.NewGuid().ToString("N"));
        ...
        public static int Main(string[] args)
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine("SteamPrototype: health | list [--page N] [--out FILE] | details --id ID | create-test/update-test/verify-test/download-test --plan FILE --state FILE | self-test; optional --timeout 5..600 --language english");
                    return 0;
                }
                ...
                for (int i = 1; i < args.Length; i += 2)
                {
                    ...
                    Require(new[] { "timeout", "language", "page", "out", "id", "plan", "state" }.Contains(key) && !Options.ContainsKey(key), "Unknown/duplicate option.");
                ...
                TestPlan plan = null;
                if (args[0].EndsWith("-test", StringComparison.Ordinal)) { plan = Read<TestPlan>(Option("plan")); ValidatePlan(plan); Option("state"); }
                ...
                Emit("session", new { version = "0.1.0", appId = App, steamId = Account, persona = SteamFriends.GetPersonaName(), runtime = Environment.Version.ToString(), x64 = Environment.Is64BitProcess, steamworks = typeof(SteamAPI).Assembly.FullName });
                switch (args[0])
                {
                    case "health": Emit("health", new { ok = true }); break;
                    case "list": ListPublished(); break;
                    case "details": Emit("details", Details(Option("id"))); break;
                    default:

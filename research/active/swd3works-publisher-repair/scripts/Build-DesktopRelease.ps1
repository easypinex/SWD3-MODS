...
$package=Join-Path $output 'SWD3-Mod-Studio-0.3.0-Windows'
...
if($desktop.Version -ne '0.3.0' -or $worker.Version -ne '0.3.0'){throw 'Expected version 0.3.0.'}
...
$files=@('SWD3ModStudio.exe','SWD3ModStudio.exe.config','desktop-fingerprint.json','worker/SteamPrototype.exe','worker/SteamPrototype.exe.config','worker/SteamReleasePackage.py','worker/steam_appid.txt')
...
@{Version='0.3.0';Files=@($inventory)} | ConvertTo-Json -Depth 5 | Set-Content (Join-Path $package 'release-files.json') -Encoding UTF8
$zip=Join-Path $output 'SWD3-Mod-Studio-0.3.0-Windows.zip'

...
$package=Join-Path $output 'SWD3-Mod-Studio-0.3.1-Windows'
...
if($desktop.Version -ne '0.3.1' -or $worker.Version -ne '0.3.0'){throw 'Expected desktop 0.3.1 / worker 0.3.0.'}
if((Get-FileHash (Join-Path $build 'SteamEntry.exe')).Hash -ne $desktop.SteamEntrySHA256){throw 'Entry fingerprint mismatch.'}
...
$files=@('SWD3ModStudio.exe','SWD3ModStudio.exe.config','SteamEntry.exe','desktop-fingerprint.json','worker/SteamPrototype.exe','worker/SteamPrototype.exe.config','worker/SteamReleasePackage.py','worker/steam_appid.txt')
...
@{Version='0.3.1';Files=@($inventory)} | ConvertTo-Json -Depth 5 | Set-Content (Join-Path $package 'release-files.json') -Encoding UTF8
$zip=Join-Path $output 'SWD3-Mod-Studio-0.3.1-Windows.zip'

...
$sources=@('StudioCore.cs','StudioApp.cs','DesktopTests.cs','SteamEntry.cs') | ForEach-Object { Join-Path $source $_ }
& $compiler /nologo /target:winexe /platform:anycpu /optimize+ /warnaserror+ "/out:$exe" /reference:System.Web.Extensions.dll /reference:System.Web.dll /reference:System.Xaml.dll @refs "/resource:$(Join-Path $source 'MainWindow.xaml'),MainWindow.xaml" @sources
...
$entry=Join-Path $output 'SteamEntry.exe'
& $compiler /nologo /target:winexe /platform:anycpu /optimize+ /warnaserror+ "/out:$entry" /reference:System.Web.Extensions.dll /reference:System.Web.dll /reference:System.Windows.Forms.dll (Join-Path $source 'StudioCore.cs') (Join-Path $source 'SteamEntry.cs') (Join-Path $source 'SteamEntryMain.cs')
if($LASTEXITCODE -ne 0 -or !(Test-Path $entry)){throw 'Steam entry compilation failed.'}
$fingerprint=[ordered]@{Version='0.3.1';RecordedAt=(Get-Date).ToString('o');ExecutableSHA256=(Get-FileHash $exe).Hash;SteamEntrySHA256=(Get-FileHash $entry).Hash;UI='WPF .NET Framework 4.8 AnyCPU';Worker='isolated x86 process';Sources=@()}
foreach ($path in @($sources)+(Join-Path $source 'MainWindow.xaml')+(Join-Path $source 'SteamEntryMain.cs')) { $fingerprint.Sources += @{Name=[IO.Path]::GetFileName($path);SHA256=(Get-FileHash $path).Hash} }

...
$sources=@('StudioCore.cs','StudioApp.cs','DesktopTests.cs') | ForEach-Object { Join-Path $source $_ }
...
$fingerprint=[ordered]@{Version='0.3.0';RecordedAt=(Get-Date).ToString('o');ExecutableSHA256=(Get-FileHash $exe).Hash;UI='WPF .NET Framework 4.8 AnyCPU';Worker='isolated x86 process';Sources=@()}
foreach ($path in @($sources)+(Join-Path $source 'MainWindow.xaml')) { $fingerprint.Sources += @{Name=[IO.Path]::GetFileName($path);SHA256=(Get-FileHash $path).Hash} }
